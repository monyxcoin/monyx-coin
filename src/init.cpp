...
bool AppInit(int argc, char *argv[]) {
    ...
    try {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help")) {
            ...
            std::string strUsage = _("Monyx version") + " " + FormatFullVersion() + "\n\n" +
                                   _("Usage:") + "\n" +
                                   "  Monyxd [options]                     " + "\n" +
                                   "  Monyxd [options] <command> [params]  " + _("Send command to -server or Monyxd") +
                                   "\n" +
                                   "  Monyxd [options] help                " + _("List commands") + "\n" +
                                   "  Monyxd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "Monyx:"))
                fCommandLine = true;
...
bool static InitError(const std::string &str) {
    uiInterface.ThreadSafeMessageBox(str, _("Monyx"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str) {
    uiInterface.ThreadSafeMessageBox(str, _("Monyx"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION |
...
std::string HelpMessage() {
    string strUsage = _("Options:") + "\n" +
                      "  -?                     " + _("This help message") + "\n" +
                      "  -conf=<file>           " + _("Specify configuration file (default: Monyx.conf)") + "\n" +
                      "  -pid=<file>            " + _("Specify pid file (default: Monyxd.pid)") + "\n" +
                      ...
                      _("Use proxy to reach tor hidden services (default: same as -proxy)") + "\n"
                              "  -torname=<host.onion>  " +
                      _("Send the specified hidden service name when connecting to Tor nodes (default: none)") + "\n"
                              "  -dns                   " +
                      _("Allow DNS lookups for -addnode, -seednode and -connect") + "\n" +
                      "  -port=<port>           " +
                      _("Listen for connections on <port> (default: 41906 or testnet: 17778)") + "\n" +
                      ...
                      "\n" +
                      "  -bantime=<n>           " +
                      _("Number of seconds to keep misbehaving peers from reconnecting (default: 86400)") + "\n" +
                      "  -maxreceivebuffer=<n>  " +
                      _("Maximum per-connection receive buffer, <n>*1000 bytes (default: 5000)") + "\n" +
                      "  -maxsendbuffer=<n>     " +
                      _("Maximum per-connection send buffer, <n>*1000 bytes (default: 1000)") + "\n" +
                      "  -detachdb              " +
                      _("Detach block and address databases. Increases shutdown time (default: 0)") + "\n" +
                      ...
                      "  -mininput=<amt>        " + str(boost::format(
            _("When creating transactions, ignore inputs with value less than this (default: %s)")) %
                                                        FormatMoney(MIN_TXOUT_AMOUNT)) + "\n" +
                      #ifdef QT_GUI
                      ...
                      "  -debug                 " +
                      _("Output extra debugging information. Implies all other -debug* options") + "\n" +
                      ...
                      "  -rpcport=<port>        " +
                      _("Listen for JSON-RPC connections on <port> (default: 41905 or testnet: 18345)") + "\n" +
                      ...
                      "  -zapwallettxes         " +
                      _("Clear list of wallet transactions (diagnostic tool; implies -rescan)") + "\n" +
                      ...
                      "\n" +
                      "  -par=N                 " +
                      _("Set the number of script verification threads (1-16, 0=auto, default: 0)") + "\n" +
                      ...
                      "  -blockprioritysize=<n> " +
                      _("Set maximum size of high-priority/low-fee transactions in bytes (default: 27000)") + "\n" +
                      ...
                      _("Acceptable ciphers (default: TLSv1+HIGH:!SSLv2:!aNULL:!eNULL:!AH:!3DES:@STRENGTH)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (strWalletFileName !=
        ...
        return InitError(strprintf(_("Wallet %s resides outside data directory %s."), strWalletFileName.c_str(),
                                   strDataDir.c_str()));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Monyx is probably already running."),
                                   strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "Monyx server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName)) {
        ...
        if (r == CDBEnv::RECOVER_OK) {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("Monyx"),
    ...
    BOOST_FOREACH(string strDest, mapMultiArgs["-seednode"])AddOneShot(strDest);
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Monyx") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE) {
            strErrors << _("Wallet needed to be rewritten: restart Monyx to complete") << "\n";
