...
public:
    ...
    CBigNum(int8_t n) {
        BN_init(this);
        if (n >= 0) setuint32(n); else setint64(n);
    }
    ...
    CBigNum(int64_t n) {
        BN_init(this);
        if (n >= 0) setuint64(n); else setint64(n);
    }
    ...
    */
    ...
    int32_t getint32() const {
        ...
        if (!BN_is_negative(this))
            return (n > (uint64_t) std::numeric_limits<int32_t>::max() ? std::numeric_limits<int32_t>::max()
                                                                       : (int32_t) n);
        else
            return (n > (uint64_t) std::numeric_limits<int32_t>::max() ? std::numeric_limits<int32_t>::min()
                                                                       : -(int32_t) n);
    }
