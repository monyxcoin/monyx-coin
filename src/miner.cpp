...
// Copyright (c) 2013 The Monyx developers
...
CBlock *CreateNewBlock(CWallet *pwallet, CTransaction *txCoinStake) {
    ...
    int64_t nFees = 0;
    {
        ...
        while (!vecPriority.empty()) {
            ...
            if (!tx.ConnectInputs(txdb, mapInputs, mapTestPoolTmp, CDiskTxPos(1, 1, 1), pindexPrev, false, true, true,
...
bool FillMap(CWallet *pwallet, uint32_t nUpperTime, MidstateMap &inputsMap) {
    ...
    CTxDB txdb("r");
    {
        ...
        CoinsSet setCoins;
        int64_t nValueIn = 0;
        if (!pwallet->SelectCoinsSimple(nBalance - nReserveBalance, MIN_TX_FEE, MAX_MONEY, nUpperTime,
                                        nCoinbaseMaturity + 10, setCoins, nValueIn))
        ...
        for (CoinsSet::const_iterator pcoin = setCoins.begin(); pcoin != setCoins.end(); pcoin++) {
            ...
            ssKernel << block.nTime << (txindex.pos.nTxPos - txindex.pos.nBlockPos) << pcoin->first->nTime
                     << pcoin->second;

            // (txid, vout.n) => (kernel, (tx.nTime, nAmount))
            inputsMap[key] = make_pair(std::vector<unsigned char>(ssKernel.begin(), ssKernel.end()),
                                       make_pair(pcoin->first->nTime, pcoin->first->vout[pcoin->second].nValue));
        }
...
bool ScanMap(const MidstateMap &inputsMap, uint32_t nBits, MidstateMap::key_type &LuckyInput,
    ...
    if (inputsMap.size() > 0 && nSearchTime > nLastCoinStakeSearchTime) {
        ...
        for (MidstateMap::const_iterator input = inputsMap.begin(); input != inputsMap.end(); input++) {
            ...
            if (ScanKernelBackward(kernel, nBits, input->second.second.first, input->second.second.second, interval,
                                   solution)) {
                // Solution found
    ...
    try {
        ...
        do {
            ...
            if (ScanMap(inputsMap, nBits, LuckyInput, solution)) {
                ...
                if (!pwallet->CreateCoinStake(LuckyInput.first, LuckyInput.second, solution.second, nBits, txCoinStake,
                                              key)) {
                    string strMessage = _(
                            "Warning: Unable to create coinstake transaction, see debug.log for the details. Mining thread has been stopped.");
                ...
                if (!pblock) {
                    string strMessage = _(
                            "Warning: Unable to allocate memory for the new block object. Mining thread has been stopped.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                ...
                if (!key.Sign(pblock->GetHash(), pblock->vchBlockSig)) {
                    string strMessage = _(
                            "Warning: Proof-of-Stake miner is unable to sign the block (locked wallet?). Mining thread has been stopped.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
