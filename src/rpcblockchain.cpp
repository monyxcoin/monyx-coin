...
double GetPoWMHashPS() {
    ...
    while (pindex) {
        if (pindex->IsProofOfWork()) {
            ...
            nTargetSpacingWork = ((nPoWInterval - 1) * nTargetSpacingWork + nActualSpacingWork + nActualSpacingWork) /
                                 (nPoWInterval + 1);
...
Object blockToJSON(const CBlock &block, const CBlockIndex *blockindex, bool fPrintTransactionDetail) {
    ...
    result.push_back(Pair("proofhash", blockindex->IsProofOfStake() ? blockindex->hashProofOfStake.GetHex()
                                                                    : blockindex->GetBlockHash().GetHex()));
    result.push_back(Pair("entropybit", (int) blockindex->GetStakeEntropyBit()));
    result.push_back(Pair("modifier", strprintf("%016"
                                                        PRIx64, blockindex->nStakeModifier)));
...
Value getrawmempool(const Array &params, bool fHelp) {
    ...
    Array a;
    BOOST_FOREACH(const uint256 &hash, vtxid)a.push_back(hash.ToString());
