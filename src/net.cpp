...
CNode *ConnectNode(CAddress addrConnect, const char *pszDest, int64_t nTimeout) {
    ...
    // Connect
    SOCKET hSocket;
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest, GetDefaultPort()) : ConnectSocket(addrConnect,
                                                                                                       hSocket)) {
...
void CNode::PushVersion() {
    ...
    printf("send version message: version %d, blocks=%d, us=%s, them=%s, peer=%s\n", PROTOCOL_VERSION, nBestHeight,
           addrMe.ToString().c_str(), addrYou.ToString().c_str(), addr.ToString().c_str());
...
bool CNode::Misbehaving(int howmuch) {
    ...
    if (nMisbehavior >= GetArgInt("-banscore", 100)) {
        ...
        printf("Misbehaving: %s (%d -> %d) DISCONNECTING\n", addr.ToString().c_str(), nMisbehavior - howmuch,
               nMisbehavior);
        {
            LOCK(cs_setBanned);
...
static const char *strDNSSeed[][2] = {
        {"node.monyx.io", "node.monyx.io"}
};

void ThreadDNSAddressSeed(void *parg) {
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("Monyx-dnsseed");
...
void ThreadOpenAddedConnections2(void *parg) {
    ...
    if (HaveNameProxy()) {
        while (!fShutdown) {
            ...
            {
                LOCK(cs_vAddedNodes);
                BOOST_FOREACH(string &strAddNode, vAddedNodes)lAddresses.push_back(strAddNode);
            }
    ...
    for (uint32_t i = 0; true; i++) {
        ...
        BOOST_FOREACH(string &strAddNode, lAddresses) {
                        ...
                        if (Lookup(strAddNode.c_str(), vservNode, GetDefaultPort(), fNameLookup, 0)) {
                            ...
                            {
                                LOCK(cs_setservAddNodeAddresses);
                                BOOST_FOREACH(CService &serv, vservNode)setservAddNodeAddresses.insert(serv);
                            }
                        }
                    }
...
bool BindListenPort(const CService &addrBind, string &strError) {
    ...
    if (::bind(hListenSocket, (struct sockaddr *) &sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Monyx is probably already running."),
    ...
    if (listen(hListenSocket, SOMAXCONN) == SOCKET_ERROR) {
        strError = strprintf("Error: Listening for incoming connections failed (listen returned error %d)",
                             WSAGetLastError());
        printf("%s\n", strError.c_str());
        closesocket(hListenSocket);
        return false;
...
public:
    ...
    ~CNetCleanup() {
        ...
        BOOST_FOREACH(CNode *pnode, vNodes)delete pnode;
        BOOST_FOREACH(CNode *pnode, vNodesDisconnected)delete pnode;
        vNodes.clear();
