...
public:
    ...
    CNode(SOCKET hSocketIn, CAddress addrIn, std::string addrNameIn = "", bool fInboundIn = false) : vSend(SER_NETWORK,
                                                                                                           MIN_PROTO_VERSION),
                                                                                                     vRecv(SER_NETWORK,
                                                                                                           MIN_PROTO_VERSION) {
        nServices = 0;
...
inline void RelayInventory(const CInv &inv) {
    ...
    {
        LOCK(cs_vNodes);
        BOOST_FOREACH(CNode *pnode, vNodes)pnode->PushInventory(inv);
    }
}
