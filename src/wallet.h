...
public:
    ...
    bool
    SelectCoinsSimple(int64_t nTargetValue, int64_t nMinValue, int64_t nMaxValue, unsigned int nSpendTime, int nMinConf,
    ...
    bool
    CreateCoinStake(uint256 &hashTx, uint32_t nOut, uint32_t nTime, uint32_t nBits, CTransaction &txNew, CKey &key);

    bool MergeCoins(const int64_t &nAmount, const int64_t &nMinValue, const int64_t &nMaxValue,
                    std::list<uint256> &listMerged);

    std::string SendMoney(CScript scriptPubKey, int64_t nValue, CWalletTx &wtxNew, bool fAskFee = false);

    std::string
    SendMoneyToDestination(const CTxDestination &address, int64_t nValue, CWalletTx &wtxNew, bool fAskFee = false);
    ...
    bool IsMine(const CTransaction &tx) const {
        BOOST_FOREACH(const CTxOut &txout, tx.vout)if (IsMine(txout) && txout.nValue >= nMinimumInputValue)
                            return true;
        return false;
    }
...
public:
    ...
    (
            ...
            if (fRead) {
                ...
                pthis->nTimeSmart = mapValue.count("timesmart") ? (unsigned int) atoi64(pthis->mapValue["timesmart"])
                                                                : 0;
            }
    ...
    void GetAmounts(int64_t &nGeneratedImmature, int64_t &nGeneratedMature,
                    ...
                    std::list<std::pair<CTxDestination, int64_t> > &listSent, int64_t &nFee,
    ...
    bool IsTrusted() const {
        ...
        for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
            ...
            if (mapPrev.empty()) {
                BOOST_FOREACH(const CMerkleTx &tx, vtxPrev)mapPrev[tx.GetHash()] = &tx;
            }
...
public:
    ...
    std::string ToString() const {
        return strprintf("COutput(%s, %d, %d, %d) [%s]", tx->GetHash().ToString().substr(0, 10).c_str(), i, fSpendable,
    ...
    (
            ...
            if (fRead) {
                ...
                if (std::string::npos != nSepPos) {
                    CDataStream ss(std::vector<char>(strComment.begin() + nSepPos + 1, strComment.end()), nType,
                                   nVersion);
                    ss >> me.mapValue;
