...
boost::filesystem::path GetDefaultDataDir() {
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Monyx
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Monyx
    // Mac: ~/Library/Application Support/Monyx
    // Unix: ~/.Monyx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Monyx";
#else
    fs::path pathRet;
...
boost::filesystem::path GetConfigFile() {
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Monyx.conf"));
...
boost::filesystem::path GetPidFile() {
    boost::filesystem::path pathPidFile(GetArg("-pid", "Monyxd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1) {
        ...
        if (abs64(nMedian) < 70 * 60) {
            ...
            if (!fDone) {
                ...
                BOOST_FOREACH(int64_t nOffset, vSorted)if (nOffset != 0 && abs64(nOffset) < 5 * 60)
                                    fMatch = true;

                if (!fMatch) {
                    fDone = true;
                    string strMessage = _(
                            "Warning: Please check that your computer's date and time are correct! If your clock is wrong Monyx will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage + " ", string("Monyx"),
