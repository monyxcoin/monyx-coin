...
CBigNum bnProofOfWorkLimit(~uint256(0)
                                   >> 20); // "standard" scrypt target limit for proof of work, results with 0,000244140625 proof-of-work difficulty
CBigNum bnProofOfStakeLegacyLimit(~uint256(0)
                                          >> 20); // proof of stake target limit from block #15000 and until 20 June 2013, results with 0,00390625 proof of stake difficulty
CBigNum bnProofOfStakeLimit(~uint256(0)
                                    >> 20); // proof of stake target limit since 20 June 2013, equal to 0.03125  proof of stake difficulty
CBigNum bnProofOfStakeHardLimit(~uint256(0)
                                        >> 20); // disabled temporarily, will be used in the future to fix minimal proof of stake difficulty at 0.25
...
unsigned int nStakeMaxAge = 3 * 365 * 24 * nOneHour; // 3 years as full weight
...
void static EraseFromWallets(uint256 hash) {
    BOOST_FOREACH(CWallet *pwallet, setpwalletRegistered)pwallet->EraseFromWallet(hash);
}
...
void SyncWithWallets(const CTransaction &tx, const CBlock *pblock, bool fUpdate, bool fConnect) {
    ...
    BOOST_FOREACH(CWallet *pwallet, setpwalletRegistered)pwallet->AddToWalletIfInvolvingMe(tx, pblock, fUpdate);
}

// notify wallets about a new best chain
void static SetBestChain(const CBlockLocator &loc) {
    BOOST_FOREACH(CWallet *pwallet, setpwalletRegistered)pwallet->SetBestChain(loc);
}

// notify wallets about an updated transaction
void static UpdatedTransaction(const uint256 &hashTx) {
    BOOST_FOREACH(CWallet *pwallet, setpwalletRegistered)pwallet->UpdatedTransaction(hashTx);
}

// dump all wallets
void static PrintWallets(const CBlock &block) {
    BOOST_FOREACH(CWallet *pwallet, setpwalletRegistered)pwallet->PrintWallet(block);
}

// notify wallets about an incoming inventory (for request counts)
void static Inventory(const uint256 &hash) {
    BOOST_FOREACH(CWallet *pwallet, setpwalletRegistered)pwallet->Inventory(hash);
}

// ask wallets to resend their transactions
void ResendWalletTransactions() {
    BOOST_FOREACH(CWallet *pwallet, setpwalletRegistered)pwallet->ResendWalletTransactions();
}
...
bool AddOrphanTx(const CTransaction &tx) {
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)mapOrphanTransactionsByPrev[txin.prevout.hash].insert(hash);
...
int64_t
CTransaction::GetMinFee(unsigned int nBlockSize, bool fAllowFree, enum GetMinFee_mode mode, unsigned int nBytes) const {
...
bool CTxMemPool::accept(CTxDB &txdb, CTransaction &tx, bool fCheckInputs,
    ...
    if (fCheckInputs) {
        ...
        if (!tx.ConnectInputs(txdb, mapInputs, mapUnused, CDiskTxPos(1, 1, 1), pindexBest, false, false, true,
                              STRICT_FLAGS)) {
...
bool CTxMemPool::remove(CTransaction &tx) {
    // Remove transaction from memory pool
    {
        ...
        uint256 hash = tx.GetHash();
        if (mapTx.count(hash)) {
            BOOST_FOREACH(const CTxIn &txin, tx.vin)mapNextTx.erase(txin.prevout);
            mapTx.erase(hash);
            nTransactionsUpdated++;
        }
    }
...
bool GetTransaction(const uint256 &hash, CTransaction &tx, uint256 &hashBlock) {
    {
        ...
        BOOST_FOREACH(PAIRTYPE(
                              const uint256, CBlockIndex*)& item, mapBlockIndex) {
...
int64_t GetProofOfWorkReward(unsigned int nBits, int64_t nFees, int nHeight) {
    int64_t nSubsidy = 65 * COIN;

    // Force block reward to zero when right shift is undefined.
    int halvings = nHeight / 100000;
    if (halvings >= 64)
        return nFees;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nTime, bool bCoinYearOnly) {
    // Determine the reward based on coin age
    int64_t nReward;
    if (nCoinAge < 6 * 31)
        nReward = 120 * CENT; // 10% for first six months
    else if (nCoinAge < 365 * 2)
        nReward = 24 * CENT; // 2% per month second year
    else
        nReward = 12 * CENT; // 1% per month third year

    int64_t nSubsidy = nCoinAge * nReward / 365;
...
unsigned int GetNextTargetRequired(const CBlockIndex *pindexLast, bool fProofOfStake) {
    CBigNum bnTargetLimit = !fProofOfStake ? bnProofOfWorkLimit : GetProofOfStakeLimit(pindexLast->nHeight,
                                                                                       pindexLast->nTime);
    ...
    if (!IsCoinBase()) {
            ...
            if (fScriptChecks) {
                ...
                if (pvChecks) {
                    ...
                    if (flags & STRICT_FLAGS) {
                        ...
                        if (check())
                            return error("ConnectInputs() : %s strict VerifySignature failed",
                                         GetHash().ToString().substr(0, 10).c_str());
                    }
        ...
        if (IsCoinStake()) {
            if (nTime > Checkpoints::GetLastCheckpointTime()) {
                ...
                int64_t nCalculatedReward = GetProofOfStakeReward(nCoinAge, pindexBlock->nBits, nTime) -
                                            GetMinFee(1, false, GMF_BLOCK, nTxSize) + CENT;

                if (nReward > nCalculatedReward)
                    return DoS(100,
                               error("ConnectInputs() : coinstake pays too much(actual=%" PRId64 " vs calculated=%" PRId64 ")",
                                     nReward, nCalculatedReward));
            }
        } else {
...
bool CBlock::DisconnectBlock(CTxDB &txdb, CBlockIndex *pindex) {
    ...
    BOOST_FOREACH(CTransaction &tx, vtx)SyncWithWallets(tx, this, false, false);
...
bool CBlock::ConnectBlock(CTxDB &txdb, CBlockIndex *pindex, bool fJustCheck) {
    ...
    bool fEnforceBIP30 = true; // Always active in Monyx
    ...
    BOOST_FOREACH(CTransaction &tx, vtx) {
                    ...
                    if (tx.IsCoinBase())
                        ...
                        if (!tx.ConnectInputs(txdb, mapInputs, mapQueuedChanges, posThisTx, pindex, true, false,
                                              fScriptChecks, SCRIPT_VERIFY_NOCACHE | SCRIPT_VERIFY_P2SH,
    ...
    BOOST_FOREACH(CTransaction &tx, vtx)SyncWithWallets(tx, this, true);
...
bool static Reorganize(CTxDB &txdb, CBlockIndex *pindexNew) {
    ...
    for (unsigned int i = 0; i < vConnect.size(); i++) {
        ...
        BOOST_FOREACH(const CTransaction &tx, block.vtx)vDelete.push_back(tx);
    }
    ...
    BOOST_FOREACH(CTransaction &tx, vResurrect)tx.AcceptToMemoryPool(txdb, false);

    // Delete redundant memory transactions that are in the connected branch
    BOOST_FOREACH(CTransaction &tx, vDelete)mempool.remove(tx);
...
bool CBlock::SetBestChainInner(CTxDB &txdb, CBlockIndex *pindexNew) {
    ...
    BOOST_FOREACH(CTransaction &tx, vtx)mempool.remove(tx);
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos) {
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016" PRIx64,
    ...
    if ((++counter & 0x0F) == 0 ||
        !IsInitialBlockDownload()) // repaint every 16 blocks if not in initial block download
        uiInterface.NotifyBlocksChanged();
    return true;
}
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const {
    ...
    if (fProofOfStake) {
        ...
        if (GetBlockTime() != (int64_t) vtx[1].nTime)
            return DoS(50, error("CheckBlock() : coinstake timestamp violation nTimeBlock=%" PRId64 " nTimeTx=%u",
                                 GetBlockTime(), vtx[1].nTime));

        // Monyx: check proof-of-stake block signature
...
bool CBlock::AcceptBlock() {
    ...
    if (pindexPrev->nHeight < LAST_POW_BLOCK && pindexPrev->nHeight > 100000 &&
    ...
    if (hashBestChain == hash) {
        ...
        BOOST_FOREACH(CNode *pnode, vNodes)
                        if (nBestHeight >
                            (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 2000
                                                          : nBlockEstimate))
...
bool
CBlockIndex::IsSuperMajority(int minVersion, const CBlockIndex *pstart, unsigned int nRequired, unsigned int nToCheck) {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes) {
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes) {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Monyx",
...
string GetWarnings(string strFor) {
    ...
    // Alerts
    {
        LOCK(cs_mapAlerts);
        BOOST_FOREACH(PAIRTYPE(
                              const uint256, CAlert)& item, mapAlerts) {
...
unsigned char pchMessageStart[4] = {0xb9, 0xf3, 0xe2, 0xe5};
...
bool static ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv) {
    ...
    if (strCommand == "version") {
        ...
        // Relay alerts
        {
            LOCK(cs_mapAlerts);
            BOOST_FOREACH(PAIRTYPE(
                                  const uint256, CAlert)& item, mapAlerts)item.second.RelayTo(pfrom);
        }
        ...
        printf("receive version message: version %d, blocks=%d, us=%s, them=%s, peer=%s\n", pfrom->nVersion,
               pfrom->nStartingHeight, addrMe.ToString().c_str(), addrFrom.ToString().c_str(),
               pfrom->addr.ToString().c_str());
        ...
        if (pfrom->setKnown.count(alertHash) == 0) {
            if (alert.ProcessAlert()) {
                ...
                {
                    LOCK(cs_vNodes);
                    BOOST_FOREACH(CNode *pnode, vNodes)alert.RelayTo(pnode);
                }
            } else {
