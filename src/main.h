...
static const uint256 hashGenesisBlock("0x0000001b1255acc9c9310c4a4559cf7c3cef26ecbe3118fe1d6c2089f92554b1");
...
public:
    ...
    bool IsFinal(int nBlockHeight = 0, int64_t nBlockTime = 0) const {
        ...
        if ((int64_t) nLockTime < ((int64_t) nLockTime < LOCKTIME_THRESHOLD ? (int64_t) nBlockHeight : nBlockTime))
            return true;
        BOOST_FOREACH(const CTxIn &txin, vin)if (!txin.IsFinal())
                            return false;
        return true;
    }
    ...
    */
    ...
    int64_t GetMinFee(unsigned int nBlockSize = 1, bool fAllowFree = false, enum GetMinFee_mode mode = GMF_BLOCK,
    ...
    std::string ToString() const {
        ...
        str += strprintf("(hash=%s, nTime=%d, ver=%d, vin.size=%"
                                 PRIszu
                                 ", vout.size=%"
                                 PRIszu
                                 ", nLockTime=%d)\n",
     ...
     */
    bool
    ConnectInputs(CTxDB &txdb, MapPrevTx inputs, std::map<uint256, CTxIndex> &mapTestPool, const CDiskTxPos &posThisTx,
...
public:
    ...
    CScriptCheck(const CTransaction &txFromIn, const CTransaction &txToIn, unsigned int nInIn, unsigned int nFlagsIn,
                 int nHashTypeIn) :
...
public:
    ...
    unsigned int GetStakeEntropyBit(unsigned int nHeight) const {
        // Protocol switch to support p2pool at Monyx block #0
        ...
        // Before Monyx block #0 - get from pregenerated table
        int nBitNum = nHeight & 0xFF;
        int nItemNum = nHeight / 0xFF;

        unsigned int nEntropyBit = (unsigned int) ((entropyStore[nItemNum] & (uint256(1) << nBitNum))
                >> nBitNum).Get64();
        if (fDebug && GetBoolArg("-printstakemodifier"))
    ...
    int64_t GetMaxTransactionTime() const {
        ...
        BOOST_FOREACH(const CTransaction &tx, vtx)maxTransactionTime = std::max(maxTransactionTime, (int64_t) tx.nTime);
        return maxTransactionTime;
    }

    uint256 BuildMerkleTree() const {
        vMerkleTree.clear();
        BOOST_FOREACH(const CTransaction &tx, vtx)vMerkleTree.push_back(tx.GetHash());
        int j = 0;
...
public:
    ...
    std::string ToString() const {
        return strprintf(
                "CBlockIndex(nprev=%p, pnext=%p, nFile=%u, nBlockPos=%-6d nHeight=%d, nMint=%s, nMoneySupply=%s, nFlags=(%s)(%d)(%s), nStakeModifier=%016"
                PRIx64
                ", nStakeModifierChecksum=%08x, hashProofOfStake=%s, prevoutStake=(%s), nStakeTime=%d merkle=%s, hashBlock=%s)",
