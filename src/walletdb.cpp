...
int64_t CWalletDB::GetAccountCreditDebit(const string &strAccount) {
    ...
    BOOST_FOREACH (const CAccountingEntry &entry, entries)nCreditDebit += entry.nCreditDebit;
            ...
            if (31404 <= wtx.fTimeReceivedIsTxTime && wtx.fTimeReceivedIsTxTime <= 31703) {
                if (!ssValue.empty()) {
                    ...
                    strErr = strprintf("LoadWallet() upgrading tx ver=%d %d '%s' %s",
                                       wtx.fTimeReceivedIsTxTime, fTmp, wtx.strFromAccount.c_str(),
                                       hash.ToString().c_str());
...
DBErrors CWalletDB::LoadWallet(CWallet *pwallet) {
    ...
    BOOST_FOREACH(uint256 hash, wss.vWalletUpgrade)WriteTx(hash, pwallet->mapWallet[hash]);
...
bool DumpWallet(CWallet *pwallet, const string &strDest) {
    ...
    while (!fShutdown) {
        ...
        // produce output
        file << strprintf("# Wallet dump created by Monyx %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
...
bool CWalletDB::Recover(CDBEnv &dbenv, std::string filename, bool fOnlyKeys) {
    ...
    // found.
    int64_t now = GetTime();
    std::string newFilename = strprintf("wallet.%"
                                                PRId64
                                                ".bak", now);
