...

    {
        ...
        BOOST_FOREACH(MasterKeyMap::value_type & pMasterKey, mapMasterKeys) {
                        ...
                        if (CCryptoKeyStore::Unlock(vMasterKey)) {
                            ...
                            pMasterKey.second.nDeriveIterations = (uint32_t) (pMasterKey.second.nDeriveIterations *
                                                                              nFirstMultiplier);
...
bool CWallet::EncryptWallet(const SecureString &strWalletPassphrase) {
    ...
    kMasterKey.nDeriveIterations = (uint32_t) (
            (kMasterKey.nDeriveIterations + kMasterKey.nDeriveIterations * nMultiplier) / 2);
        ...
        BOOST_FOREACH(const CTxIn &txin, tx.vin) {
                        ...
                        if (mi != mapWallet.end()) {
                            ...
                            if (txin.prevout.n >= wtx.vout.size())
                                ...
                                printf("WalletUpdateSpent found spent coin %sMYX %s\n",
...
void CWallet::MarkDirty() {
    {
        ...
        BOOST_FOREACH(PAIRTYPE(
                              const uint256, CWalletTx)& item, mapWallet)item.second.MarkDirty();
    }
}
...
bool CWallet::AddToWallet(const CWalletTx &wtxIn) {
    uint256 hash = wtxIn.GetHash();
    {
        ...
        //// debug print
        printf("AddToWallet %s  %s%s\n", wtxIn.GetHash().ToString().substr(0, 10).c_str(), (fInsertedNew ? "new" : ""),
               (fUpdated ? "update" : ""));
...
void CWalletTx::GetAccountAmounts(const string &strAccount, int64_t &nGenerated, int64_t &nReceived,
    ...
    if (strAccount == strSentAccount) {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& s, listSent)nSent += s.second;
        nFee = allFee;
    }
    {
        LOCK(pwallet->cs_wallet);
...
void CWalletTx::AddSupportingTransactions(CTxDB &txdb) {
    ...
    if (SetMerkleBranch() < COPY_DEPTH) {
        ...
        BOOST_FOREACH(const CTxIn &txin, vin)vWorkQueue.push_back(txin.prevout.hash);
        ...
        // This critsect is OK because txdb is already open
        {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
                ...
                if (mi != pwallet->mapWallet.end()) {
                    tx = (*mi).second;
                    BOOST_FOREACH(const CMerkleTx &txWalletPrev,
                                  (*mi).second.vtxPrev)mapWalletPrev[txWalletPrev.GetHash()] = &txWalletPrev;
                ...
                if (nDepth < COPY_DEPTH) {
                    BOOST_FOREACH(const CTxIn &txin, tx.vin)vWorkQueue.push_back(txin.prevout.hash);
                }
            }
        }
    }
    ...
    while (fRepeat) {
        ...
        BOOST_FOREACH(PAIRTYPE(
                        ...
                        if (txdb.ReadTxIndex(wtx.GetHash(), txindex)) {
                            ...
                            if (fUpdated) {
                                printf("ReacceptWalletTransactions found spent coin %sMYX %s\n",
                                       FormatMoney(wtx.GetCredit(MINE_ALL)).c_str(), wtx.GetHash().ToString().c_str());
                                wtx.MarkDirty();
                                wtx.WriteToDisk();
                            }
...
static void
ApproximateBestSubset(vector<pair<int64_t, pair<const CWalletTx *, unsigned int> > > vValue, int64_t nTotalLower,
...
bool CWallet::SelectCoinsSimple(int64_t nTargetValue, int64_t nMinValue, int64_t nMaxValue, unsigned int nSpendTime,
                                int nMinConf, set<pair<const CWalletTx *, unsigned int> > &setCoinsRet,
                                int64_t &nValueRet) const {
    vector<COutput> vCoins;
...
bool
CWallet::CreateTransaction(const vector<pair<CScript, int64_t> > &vecSend, CWalletTx &wtxNew, CReserveKey &reservekey,
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true) {
                ...
                double dPriority = 0;
                // vouts to the payees
                BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)wtxNew.vout.push_back(
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl))
                    return false;
                BOOST_FOREACH(PAIRTYPE(
                                      const CWalletTx*, unsigned int) pcoin, setCoins) {
                ...
                // Fill vin
                BOOST_FOREACH(const PAIRTYPE(const CWalletTx *, unsigned int) &
                                      coin, setCoins)wtxNew.vin.push_back(CTxIn(coin.first->GetHash(), coin.second));

                // Sign
                int nIn = 0;
                BOOST_FOREACH(const PAIRTYPE(const CWalletTx *, unsigned int) &
                                      coin, setCoins)if (!SignSignature(*this, *coin.first, wtxNew, nIn++))
                                    return false;
                ...
                int64_t
                nPayFee = nTransactionFee * (1 + (int64_t)
                nBytes / 1000);
...
bool CWallet::MergeCoins(const int64_t &nAmount, const int64_t &nMinValue, const int64_t &nOutputValue,
...
*/
                    ...
                    int64_t nBytes = ::GetSerializeSize(*(CTransaction *) &wtxNew, SER_NETWORK, PROTOCOL_VERSION) +
                                     wtxNew.vin.size() * 110;
    ...
    if (wtxNew.vout[0].nValue > 0) {
        int64_t nBytes =
                ::GetSerializeSize(*(CTransaction *) &wtxNew, SER_NETWORK, PROTOCOL_VERSION) + wtxNew.vin.size() * 110;
...
bool
CWallet::CreateCoinStake(uint256 &hashTx, uint32_t nOut, uint32_t nGenerationTime, uint32_t nBits, CTransaction &txNew,
                         CKey &key) {
    CWalletTx wtx;
    ...
    int64_t nCombineThreshold =
            GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nBits, 0, pindexBest->nHeight) / 3;
    ...
    int64_t nValueIn = 0;
    CoinsSet setCoins;
    if (!SelectCoinsSimple(nBalance - nReserveBalance, MIN_TX_FEE, MAX_MONEY, nGenerationTime, nCoinbaseMaturity + 10,
                           setCoins, nValueIn))
        return false;
...
bool CWallet::SetAddressBookName(const CTxDestination &address, const string &strName) {
    ...
    NotifyAddressBookChanged(this, address, strName, ::IsMine(*this, address) != MINE_NO,
...
void CWallet::PrintWallet(const CBlock &block) {
    {
        ...
        if (block.IsProofOfStake() && mapWallet.count(block.vtx[1].GetHash())) {
            ...
            printf("    PoS: %d  %d  %" PRId64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(),
                   wtx.GetCredit(MINE_ALL));
        } else if (mapWallet.count(block.vtx[0].GetHash())) {
            CWalletTx &wtx = mapWallet[block.vtx[0].GetHash()];
            printf("    PoW:  %d  %d  %" PRId64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(),
                   wtx.GetCredit(MINE_ALL));
        }
    }
    printf("\n");
...
bool CWallet::NewKeyPool(unsigned int nSize) {
    {
        LOCK(cs_wallet);
        CWalletDB walletdb(strWalletFile);
        BOOST_FOREACH(int64_t nIndex, setKeyPool)walletdb.ErasePool(nIndex);
        setKeyPool.clear();
...
set<set<CTxDestination> > CWallet::GetAddressGroupings() {
                    ...
                    if (pcoin->vin.size() > 0 && IsMine(pcoin->vin[0])) {
                        ...
                        BOOST_FOREACH(CTxIn txin, pcoin->vin) {
                                        ...
                                        if (!ExtractDestination(
                                                mapWallet[txin.prevout.hash].vout[txin.prevout.n].scriptPubKey,
                                                address))
                                            continue;
                        ...
                        BOOST_FOREACH(CTxOut txout, pcoin->vout)if (IsChange(txout)) {
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, groupings) {
                    ...
                    BOOST_FOREACH(CTxDestination address, grouping)if ((it = setmap.find(address)) != setmap.end())
                    ...
                    // update setmap
                    BOOST_FOREACH(CTxDestination element, *merged)setmap[element] = merged;
                }
...
void CWallet::FixSpentCoins(int &nMismatchFound, int64_t &nBalanceInQuestion, bool fCheckOnly) {
    ...
    BOOST_FOREACH(CWalletTx *pcoin, vCoins) {
                    ...
                    for (unsigned int n = 0; n < pcoin->vout.size(); n++) {
                        if (IsMine(pcoin->vout[n]) && pcoin->IsSpent(n) &&
                        ...
                        } else if (IsMine(pcoin->vout[n]) && !pcoin->IsSpent(n) &&
                                   (txindex.vSpent.size() > n && !txindex.vSpent[n].IsNull())) {
                    ...
                    if (IsMine((CTransaction) *pcoin) && (pcoin->IsCoinBase() || pcoin->IsCoinStake()) &&
        ...
        if (blit != mapBlockIndex.end() && blit->second->IsInMainChain()) {
            ...
            BOOST_FOREACH(const CTxOut &txout, wtx.vout) {
                            ...
                            BOOST_FOREACH(const CKeyID &keyid, vAffected) {
                                            ...
                                            std::map<CKeyID, CBlockIndex *>::iterator rit = mapKeyFirstBlock.find(
                                                    keyid);
