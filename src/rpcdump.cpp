...
Value importprivkey(const Array &params, bool fHelp) {
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
                "importprivkey <Monyxprivkey> [label] [rescan=true]\n"
...
Value importaddress(const Array &params, bool fHelp) {
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Monyx address or script");
    }
...
Value removeaddress(const Array &params, bool fHelp) {
    ...
    if (::IsMine(*pwalletMain, script) == MINE_SPENDABLE)
        throw JSONRPCError(RPC_WALLET_ERROR,
                           "The wallet contains the private key for this address or script - can't remove it");
...
Value dumpprivkey(const Array &params, bool fHelp) {
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "dumpprivkey <Monyxaddress>\n"
                        "Reveals the private key corresponding to <Monyxaddress>.");
