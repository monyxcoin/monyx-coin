...
bool CTxDB::LoadBlockIndex() {
    ...
    BOOST_FOREACH(const PAIRTYPE(uint256, CBlockIndex
                          *)& item, mapBlockIndex) {
                    CBlockIndex * pindex = item.second;
                    vSortedByHeight.push_back(make_pair(pindex->nHeight, pindex));
                }
    sort(vSortedByHeight.begin(), vSortedByHeight.end());
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex *) &
                          item, vSortedByHeight) {
                    CBlockIndex * pindex = item.second;
                    pindex->nChainTrust = (pindex->pprev ? pindex->pprev->nChainTrust : 0) + pindex->GetBlockTrust();
                    // Monyx: calculate stake modifier checksum
                            ...
                            if (ReadTxIndex(hashTx, txindex)) {
                                ...
                                if (nCheckLevel > 3) {
                                    BOOST_FOREACH(const CDiskTxPos &txpos, txindex.vSpent) {
                                                    if (!txpos.IsNull()) {
                                                        pair<unsigned int, unsigned int> posFind = make_pair(
                                                                txpos.nFile, txpos.nBlockPos);
                                                        ...
                                                        if (nCheckLevel > 5) {
                                                            ...
                                                            if (!txSpend.ReadFromDisk(txpos)) {
                                                                ...
                                                                bool fFound = false;
                                                                BOOST_FOREACH(const CTxIn &txin, txSpend.vin)if (
