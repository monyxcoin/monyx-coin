...
Value getmininginfo(const Array &params, bool fHelp) {
    ...
    obj.push_back(Pair("blockvalue", (uint64_t) GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nBits, 0,
                                                                     pindexBest->nHeight)));
    ...
    obj.push_back(Pair("stakeinterest", GetProofOfStakeReward(0, GetLastBlockIndex(pindexBest, true)->nBits,
                                                              GetLastBlockIndex(pindexBest, true)->nTime, true)));
...
Value scaninput(const Array &params, bool fHelp) {
    ...
    if (GetTransaction(hash, tx, hashBlock)) {
        ...
        if (inputs_v.type() == array_type) {
            ...
            BOOST_FOREACH(const Value &v_out, inputs) {
                            ...
                            if (nOut < 0 || nOut > (int) tx.vout.size() - 1) {
                                ...
                                strErrorMsg
                                        << boost::format("Invalid parameter, input number %d is out of range") % nOut;
        ...
        BOOST_FOREACH(const int &nOut, vInputs) {
                        ...
                        if (ScanKernelForward((unsigned char *) &itK[0], nBits, tx.nTime, tx.vout[nOut].nValue,
                                              interval, result)) {
...
Value getworkex(const Array &params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Monyx is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Monyx is downloading blocks...");
    ...
    if (params.size() == 0) {
        ...
        if (pindexPrev != pindexBest ||
            ...
            if (pindexPrev != pindexBest) {
                ...
                mapNewBlock.clear();
                BOOST_FOREACH(CBlock *pblock, vNewBlock)delete pblock;
                vNewBlock.clear();
            }
...
Value getwork(const Array &params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Monyx is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Monyx is downloading blocks...");
