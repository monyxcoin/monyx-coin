...
vector<KernelRecord> KernelRecord::decomposeOutput(const CWallet *wallet, const CWalletTx &wtx) {
    ...
    int64_t nDayWeight = (min((GetAdjustedTime() - nTime), (int64_t) (nStakeMaxAge + nStakeMinAge)) -
                          nStakeMinAge); // DayWeight * 86400, чтобы был
...
double KernelRecord::getProbToMintStake(double difficulty, int timeOffset) const {
    ...
    int64_t Weight = (min((GetAdjustedTime() - nTime) + timeOffset, (int64_t) (nStakeMinAge + nStakeMaxAge)) -
                      nStakeMinAge);
