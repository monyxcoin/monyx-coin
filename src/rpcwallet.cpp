...
void WalletTxToJSON(const CWalletTx &wtx, Object &entry) {
    ...
    BOOST_FOREACH(const PAIRTYPE(string, string)& item, wtx.mapValue)entry.push_back(Pair(item.first, item.second));
}
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
                "getnewaddress [account]\n"
                        "Returns a new Monyx address for receiving payments.  "
...
CBitcoinAddress GetAccountAddress(string strAccount, bool bForceNew = false) {
    ...
    if (account.vchPubKey.IsValid()) {
        ...
        for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin();
            ...
            BOOST_FOREACH(const CTxOut &txout, wtx.vout)if (txout.scriptPubKey == scriptPubKey)
                                bKeyUsed = true;
        }
    }
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "getaccountaddress <account>\n"
                        "Returns the current Monyx address for receiving payments to this account.");
...
Value setaccount(const Array &params, bool fHelp) {
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                "setaccount <Monyxaddress> <account>\n"
                        "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Monyx address");
...
Value mergecoins(const Array &params, bool fHelp) {
    ...
    Array mergedHashes;
    BOOST_FOREACH(const uint256 txHash, listMerged)mergedHashes.push_back(txHash.GetHex());
...
Value sendtoaddress(const Array &params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
                "sendtoaddress <Monyxaddress> <amount> [comment] [comment-to]\n"
                    ...
                    BOOST_FOREACH(CTxDestination address, grouping) {
                                    ...
                                    addressInfo.push_back(ValueFromAmount(balances[address]));
                                    {
                                        ...
                                        if (pwalletMain->mapAddressBook.find(CBitcoinAddress(address).Get()) !=
                                            ...
                                            addressInfo.push_back(pwalletMain->mapAddressBook.find(
                                                    CBitcoinAddress(address).Get())->second);
                                    }
...
Value signmessage(const Array &params, bool fHelp) {
    if (fHelp || params.size() != 2)
        throw runtime_error(
                "signmessage <Monyxaddress> <message>\n"
...
Value verifymessage(const Array &params, bool fHelp) {
    if (fHelp || params.size() != 3)
        throw runtime_error(
                "verifymessage <Monyxaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array &params, bool fHelp) {
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                "getreceivedbyaddress <Monyxaddress> [minconf=1]\n"
                        "Returns the total amount received by <Monyxaddress> in transactions with at least [minconf] confirmations.");
    ...
    if (params[0].get_str() == "*") {
        ...
        for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin();
            ...
            wtx.GetAmounts(allGeneratedImmature, allGeneratedMature, listReceived, listSent, allFee, strSentAccount,
                           filter);
            if (wtx.GetDepthInMainChain() >= nMinDepth) {
                BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r, listReceived)nBalance += r.second;
            }
            BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r, listSent)nBalance -= r.second;
            nBalance -= allFee;
...
Value sendfrom(const Array &params, bool fHelp) {
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
                "sendfrom <fromaccount> <toMonyxaddress> <amount> [minconf=1] [comment] [comment-to]\n"
                        "<amount> is a real and is rounded to the nearest " + FormatMoney(nMinimumInputValue)
                + HelpRequiringPassphrase());
...
Value sendmany(const Array &params, bool fHelp) {
    ...
    BOOST_FOREACH(const Pair &s, sendTo) {
                    ...
                    if (!address.IsValid())
                        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Monyx address: ") + s.name_);
...
Value addmultisigaddress(const Array &params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
                ...
                "each key is a Monyx address or hex-encoded public key\n"
    ...
    if ((int) keys.size() < nRequired)
        throw runtime_error(
                strprintf("not enough keys supplied "
                                  "(got %"
                                  PRIszu
                                  " keys, but need at least %d to redeem)", keys.size(), nRequired));
    if (keys.size() > 16)
        throw runtime_error(
    ...
    if (inner.size() > MAX_SCRIPT_ELEMENT_SIZE)
        throw runtime_error(
                strprintf("redeemScript exceeds size limit: %"
                                  PRIszu
                                  " > %d", inner.size(), MAX_SCRIPT_ELEMENT_SIZE));
...
Value listaccounts(const Array &params, bool fHelp) {
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin();
        ...
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& s,
                      listSent)mapAccountBalances[strSentAccount] -= s.second;
        if (wtx.GetDepthInMainChain() >= nMinDepth) {
            mapAccountBalances[""] += nGeneratedMature;
            BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& r,
                          listReceived)if (pwalletMain->mapAddressBook.count(r.first))
    ...
    BOOST_FOREACH(const CAccountingEntry &entry, acentries)mapAccountBalances[entry.strAccount] += entry.nCreditDebit;
...
Value encryptwallet(const Array &params, bool fHelp) {
    ...
    StartShutdown();
    return "wallet encrypted; Monyx server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
public:
    ...
    Object operator()(const CScriptID &scriptID) const {
        ...
        if (mine == MINE_SPENDABLE) {
            ...
            Array a;
            BOOST_FOREACH(const CTxDestination &addr, addresses)a.push_back(CBitcoinAddress(addr).ToString());
...
Value validateaddress(const Array &params, bool fHelp) {
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "validateaddress <Monyxaddress>\n"
                        "Return information about <Monyxaddress>.");
...
// Monyx: resend unconfirmed wallet transactions
