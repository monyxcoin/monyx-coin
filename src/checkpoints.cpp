...
namespace Checkpoints {
    ...
    static MapCheckpoints mapCheckpoints =
            boost::assign::map_list_of
                    (0, std::make_pair(hashGenesisBlock, 1517504499))
                    (42, std::make_pair(uint256("0x00000087a735b025721f22189debff6f153716498d2df2f5cd39193acf7efa42"),
                                        1517513474));
    ...
    uint256 AutoSelectSyncCheckpoint() {
        ...
        while (pindex->pprev && (pindex->GetBlockTime() + CHECKPOINT_MAX_SPAN > pindexBest->GetBlockTime() ||
                                 pindex->nHeight + 8 > pindexBest->nHeight))
            pindex = pindex->pprev;
    ...
    if (!mapBlockIndex.count(hashCheckpoint)) {
        ...
        if (pfrom) {
            ...
            pfrom->AskFor(CInv(MSG_BLOCK,
                               mapOrphanBlocks.count(hashCheckpoint) ? WantedByOrphan(mapOrphanBlocks[hashCheckpoint])
                                                                     : hashCheckpoint));
        }
        return false;
    }
