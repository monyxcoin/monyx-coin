...
Value getaddednodeinfo(const Array &params, bool fHelp) {
    ...
    if (params.size() == 1) {
        LOCK(cs_vAddedNodes);
        BOOST_FOREACH(string &strAddNode, vAddedNodes)laddedNodes.push_back(strAddNode);
    } else {
        string strNode = params[1].get_str();
        LOCK(cs_vAddedNodes);
        BOOST_FOREACH(string &strAddNode, vAddedNodes)if (strAddNode == strNode) {
    ...
    if (!fDns) {
        Object ret;
        BOOST_FOREACH(string &strAddNode, laddedNodes)ret.push_back(Pair("addednode", strAddNode));
        return ret;
    }
    ...
    for (list<pair<string, vector<CService> > >::iterator it = laddedAddreses.begin();
        ...
        BOOST_FOREACH(CService &addrNode, it->second) {
                        ...
                        BOOST_FOREACH(CNode *pnode, vNodes)if (pnode->addr == addrNode) {
                                            fFound = true;
                                            fConnected = true;
