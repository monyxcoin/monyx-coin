...
Alternatively, install Qt Creator and open the `Monyx-qt.pro` file.

An executable named `Monyx-qt` will be built.
...
A warning for people using the *static binary* version of Monyx on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Monyx is linked against libdb5.3.
...
installed causes Monyx-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
